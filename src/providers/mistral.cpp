...
constexpr auto kDefaultBaseUrl = "https://api.mistral.ai";
constexpr auto kDefaultModel = "mistral-large-latest";
constexpr auto kCompletionsPath = "/v1/chat/completions";

/// Convert a unified Role to the Mistral role string (same as OpenAI).
...
auto MistralProvider::sanitize_tool_call_id(std::string_view id) -> std::string {
    ...
    constexpr size_t kStrict9Len = 9;
    if (sanitized.size() < kStrict9Len) {
        // Pad with zeros
        sanitized.append(kStrict9Len - sanitized.size(), '0');
    } else if (sanitized.size() > kStrict9Len) {
        sanitized.resize(kStrict9Len);
    }

    return sanitized;
}

MistralProvider::MistralProvider(boost::asio::io_context& ioc,
...
{
    LOG_INFO("Mistral provider initialized (model: {}, base: {})",
             default_model_,
...
auto MistralProvider::convert_message(const Message& msg) const -> json {
    json j;
    ...
    if (msg.role == Role::Tool) {
        for (const auto& block : msg.content) {
            if (block.type == "tool_result") {
                j["tool_call_id"] = sanitize_tool_call_id(
                    block.tool_use_id.value_or(""));
...
auto MistralProvider::build_request_body(const CompletionRequest& req,
...
auto MistralProvider::parse_response(const std::string& body) const
...
auto MistralProvider::parse_sse_chunk(const json& chunk,
                                        CompletionResponse& response,
...
auto MistralProvider::complete(CompletionRequest req)
    -> boost::asio::awaitable<Result<CompletionResponse>> {
    auto body = build_request_body(req, false);

    LOG_DEBUG("Mistral complete request: model={}", body.value("model", ""));
...
auto MistralProvider::stream(CompletionRequest req, StreamCallback cb)
    -> boost::asio::awaitable<Result<CompletionResponse>> {
    auto body = build_request_body(req, true);

    LOG_DEBUG("Mistral stream request: model={}", body.value("model", ""));
    ...
    for (const auto& line : sse_lines) {
        ...
        try {
            ...
            LOG_WARN("Failed to parse Mistral SSE chunk: {}", e.what());
            continue;
        }
    }
...
auto MistralProvider::name() const -> std::string_view {
    return "mistral";
}

auto MistralProvider::models() const -> std::vector<std::string> {
    return {
