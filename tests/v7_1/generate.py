#!/usr/bin/env python3
"""
v2026.7.1 OpenClaw test parity generator.

Walks the OpenClaw v2026.7.1 submodule and emits one Catch2 test file per
...
Usage:
    cd <repo>/agent/mylobsterpp/tests/v7_1
...
# `__screenshots__/` matters because Playwright snapshot dirs are themselves
# named `<something>.test.ts/`, so rglob("*.test.ts") yields directories.
SKIP_PATTERN = re.compile(
    r"(node_modules|/dist/|/dist-runtime/|/vendor/|__screenshots__/|"
...
def scan_upstream_tests() -> list[Path]:
    ...
    for root in SCAN_ROOTS:
        ...
        for p in base.rglob("*.test.ts"):
            # Snapshot dirs are named `*.test.ts/` — rglob matches them too.
            if not p.is_file():
                continue
...
def relative_include_to_parity(out_path: Path) -> str:
    """Return a relative include path from out_path to v7_1/parity.hpp."""
...
def render_test_case(
    ...
    name = f"v7.1 [{upstream_rel}:{upstream_line}] {description}"
...
def render_file(upstream_rel: str, it_blocks: list[tuple[int, str]], tags: list[str], parity_inc: str) -> str:
    header = (
        f"// v2026.7.1 OpenClaw parity mirror of {upstream_rel}\n"
        f"//\n"
        f"// Auto-generated by tests/v7_1/generate.py from the openclaw v2026.7.1\n"
    ...
    if not it_blocks:
        cases.append(
            f'TEST_CASE("v7.1 [{cpp_string_escape(upstream_rel)}] empty upstream test file",\n'
