...
// v2026.5.2 OpenClaw test parity helpers.
//
// Each upstream Vitest file (`extensions/<id>/<name>.test.ts`) is mirrored by a
// Catch2 file under `tests/v5_2/<path>/<name>.cpp`. Behavioral assertions that
// mylobsterpp's C++ surface can express are ported as live Catch2
// SECTION/REQUIRE checks. Upstream assertions that depend on plugin-SDK
// abstractions mylobsterpp does not have surface as PARITY_GAP() FAILs so the
// red bar is the v5.2 parity backlog.
//
...
#define PARITY_GAP(upstream_ref, missing, note)                                              \
    FAIL("v5.2 parity gap [" missing "] (" upstream_ref "): " note)

// Tag aliases used for consistency:
//   [v5_2]         — every ported test
