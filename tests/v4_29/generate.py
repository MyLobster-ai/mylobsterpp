#!/usr/bin/env python3
"""
v2026.4.29 OpenClaw test parity generator.

Walks the OpenClaw v2026.4.29 submodule and emits one Catch2 test file per
...
sentinel `// MYLOBSTERPP_HAND_AUTHORED` is skipped. Edit those by hand to
upgrade gaps into real behavioral assertions; re-running the generator will
not stomp them.

Usage:
    cd <repo>/agent/mylobsterpp/tests/v4_29
...
def render_file(upstream_rel: str, it_blocks: list[tuple[int, str]], tags: list[str], parity_inc: str) -> str:
    header = (
        f"// v2026.4.29 OpenClaw parity mirror of {upstream_rel}\n"
        f"//\n"
        f"// Auto-generated by tests/v4_29/generate.py from the openclaw v2026.4.29\n"
    ...
    if not it_blocks:
        cases.append(
            f'TEST_CASE("v4.29 [{cpp_string_escape(upstream_rel)}] empty upstream test file",\n'
